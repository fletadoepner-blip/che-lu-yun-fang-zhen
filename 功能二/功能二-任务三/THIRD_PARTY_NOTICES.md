...
本文件声明「功能二-任务三：平台部署与性能优化」代码包中使用到的第三方组件、数据来源及其许可证，用于满足挑战杯 XH-202613 提交材料 **D10「第三方来源与许可证」** 的合规性要求。
...
许可证查证结论（已通过 GitHub API / README / Registry 镜像 config 交叉核对）：

- UGAT、FRAP 仓库**均无 LICENSE 文件、未声明开源许可证**，代码默认受版权保护（All Rights Reserved）。学术使用分别引用 Da et al., CDC 2023 与 Zheng et al., CIKM 2019。
- `danielda1/ugat:latest` 镜像 Python 为 **3.10.6**（非 3.11+），内置 CityFlow（Apache-2.0）与 SUMO（EPL-2.0），Docker Hub 页面无独立许可证或合规声明。
- 注意：UGAT 官方仓库**未发布预训练权重**。本包 `official_ugat_best.pt`（6058 字节，md5 `1be74408fd37abc7ef6e5daffdc6688b`）经哈希核对，与镜像内置 `-1_0.pt`（md5 `0fe71b6c…`）及 `centralizedGAT` 分支 `260_0.pt`（md5 `c5c416f3…`）**均不匹配**，为团队侧产物，来源需内部确认。

待办：

- [ ] 确认 `official_ugat_best.pt` 权重的确切来源（哈希核对已排除镜像 `-1_0.pt` 与分支 `260_0.pt`；需问团队是自行训练还是作者私下提供）
- [ ] 如竞赛包需再分发 UGAT / FRAP 源码或权重文件，建议书面联系作者（Longchao Da / DaRL Lab, ASU；Guanjie Zheng）确认授权
- [ ] 报告正文引用上述两篇论文（BibTeX 见附录）
