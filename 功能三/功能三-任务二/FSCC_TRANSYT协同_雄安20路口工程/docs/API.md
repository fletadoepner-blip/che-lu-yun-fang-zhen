...
运行器将需求注入阶段和注入完成后的排空阶段明确区分。`demand_end_time_s` 是最后一辆计划车辆的注入时刻；当 `scheduled_vehicles == total_demand` 后，记录字段 `demand_complete=true`、`phase=draining`，此时仿真时间继续推进仅用于让已注入车辆驶离或消化队列，不再产生新车辆。动态监视器显示 `DRAINING (injection complete)`，避免把排空阶段误认为仍在注入。
