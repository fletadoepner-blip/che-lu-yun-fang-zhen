...
def main() -> None:
    ap = argparse.ArgumentParser(); ap.add_argument("--period", choices=("morning", "midday", "evening"), default="morning"); ap.add_argument("--flow-file", default=None, help="Optional flow JSON filename under data/xiong_an_20")
    ...
    if not (data / config["flowFile"]).exists(): raise SystemExit(f"Flow file not found: {data / config['flowFile']}")
