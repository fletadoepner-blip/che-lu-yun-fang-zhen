...
def main() -> None:
    ap = argparse.ArgumentParser(); ap.add_argument("trace", type=Path); ap.add_argument("--metrics", type=Path, default=Path("outputs/metrics.csv")); args = ap.parse_args()
    trace = json.loads(args.trace.read_text(encoding="utf-8"));
    if not trace: raise SystemExit("Trace is empty; no result window can be shown.")
    name = args.trace.stem.removeprefix("trace_"); algorithm, period = name.rsplit("_", 1)
    ...
    values = [("仿真步数", metric.get("steps", "-")), ("执行速度", f"{metric.get('steps_per_second', '-')} step/s"), ("结束在网车辆", metric.get("final_active_vehicles", "-")), ("结束排队代理量", metric.get("final_queue_proxy", "-"))]
