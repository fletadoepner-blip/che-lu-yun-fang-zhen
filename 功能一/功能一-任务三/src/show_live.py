...
class LiveMonitor:
    ...
    def draw(self, data):
        ...
        suffix = "  COMPLETE" if data.get("complete") else "  RUNNING"; self.info.configure(text=f"simulation time={data['time_s']} s | on-network={data['active_vehicles']} | injected={data.get('scheduled_vehicles', 0)}/{data.get('total_demand', 0)} | queued={data['queue_proxy']} | displayed={len(data.get('vehicles', []))}{suffix}")
