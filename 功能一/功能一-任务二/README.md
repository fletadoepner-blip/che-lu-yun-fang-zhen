...
本任务用于验收的固定配时与自适应控制对比工程位于 `路口2_SUMO仿真验证/01_仿真工程/`。在当前任务根目录中，推荐双击 `run_task2.bat`；也可在 PowerShell 中执行：

```powershell
& ".\run_task2.ps1"
```

启动脚本仅使用 ASCII 文件名和 ASCII 内容，以兼容 Windows PowerShell 编码设置。请执行脚本文件，不要将 `.ps1` 文件内容逐行粘贴到终端。完整说明见 `路口2_SUMO仿真验证/01_仿真工程/README_运行说明.md`。

当前提交版性能对比图位于 `路口2_SUMO仿真验证/01_仿真工程/results/performance_comparison.png`，并已同步嵌入 Word 提交说明报告。
