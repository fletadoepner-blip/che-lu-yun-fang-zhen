...
from algorithms import ActuatedPressureAlgorithm, CoordinatedFixedAlgorithm, FixedTimeAlgorithm, MaxPressureAlgorithm
...
def build_algorithm(name: str, count: int, checkpoint: Path, grid_positions: list[list[int]] | None = None, min_green: int = 2, max_green: int = 6, switch_ratio: float = 1.10, fixed_offset: int = 0, coordination_scale: int = 3):
    ...
    if name == "fixed_10_alt": return FixedTimeAlgorithm(count, 1, fixed_offset, (0, 1, 4, 5, 6, 7, 2, 3))
    if name == "fixed_10_rev": return FixedTimeAlgorithm(count, 1, fixed_offset, (7, 6, 5, 4, 3, 2, 1, 0))
    if name == "coordinated_fixed": return CoordinatedFixedAlgorithm(grid_positions or [[0,0]]*count, offset_scale=coordination_scale)
    ...
    if name in ("ugat_frap", "ugat_only", "frap_only", "ugat_frap_no_frap", "ugat_frap_no_ugat"):
        import torch
        ...
        branch_weight = {"ugat_frap": 1.0, "ugat_only": 0.0, "frap_only": 1.0, "ugat_frap_no_frap": 1.0, "ugat_frap_no_ugat": 1.0}[name]
        ...
        class TorchAdapter:
            ...
            def choose(self, state):
                ...
                if algorithm_name == "frap_only":
                    ...
                    # Fair ablations retain this exact execution wrapper and
                    # remove only one normalized model branch.
                    ugat_term = 0.0 if algorithm_name == "ugat_frap_no_ugat" else zscore(ugat_q)
                    frap_term = 0.0 if algorithm_name == "ugat_frap_no_frap" else 0.5 * zscore(frap_q)
                    fused_q = ugat_term + frap_term + 1.0 * zscore(pressure_q)
...
def main() -> None:
    ...
    ap.add_argument("--algorithm", choices=("fixed", "fixed_10", "fixed_10_alt", "fixed_10_rev", "coordinated_fixed", "fixed_20", "fixed_40", "max_pressure", "actuated_pressure", "ugat_frap", "ugat_only", "frap_only", "ugat_frap_no_frap", "ugat_frap_no_ugat"), default="ugat_frap")
    ...
    ap.add_argument("--flow-file", type=str, default=None, help="Optional flow JSON filename under data/xiong_an_20.")
    ...
    ap.add_argument("--min-green-decisions", type=int, default=2); ap.add_argument("--max-green-decisions", type=int, default=6); ap.add_argument("--switch-ratio", type=float, default=1.10); ap.add_argument("--fixed-offset", type=int, choices=range(8), default=0); ap.add_argument("--coordination-scale", type=int, choices=range(8), default=3); args = ap.parse_args()
    try: import cityflow
    ...
    controller = build_algorithm(args.algorithm, 20, ROOT / "model" / "frozen_ugat_4x4.pt", grid_positions, args.min_green_decisions, args.max_green_decisions, args.switch_ratio, args.fixed_offset, args.coordination_scale); trace = []; started = time.perf_counter()
